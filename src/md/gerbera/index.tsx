...
function plane(width: number, height: number) {
  return function (u: number, v: number, target: any) {
    const x = (u - 0.5) * width;
...
let clothGeometry: THREE.ParametricBufferGeometry;
...
class Particle {
  ...
  integrate(scalar: number) {
    const newPos = this.tmp.subVectors(this.position, this.previous);
    newPos.multiplyScalar(DRAG).add(this.position);
    newPos.add(this.a.multiplyScalar(scalar));
